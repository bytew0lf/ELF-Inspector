...
echo "[1/6] Unit tests (dotnet test)"
"$ROOT_DIR/scripts/run_unit_tests.sh"

echo "[2/6] Conformance matrix (P0)"
"$ROOT_DIR/scripts/run_p0_conformance_matrix.sh"

echo "[3/6] Parser hardening matrix (P3)"
"$ROOT_DIR/scripts/run_p3_test_matrix.sh"

echo "[4/6] Advanced feature matrix (P4)"
"$ROOT_DIR/scripts/run_p4_test_matrix.sh"

echo "[5/6] Coverage map drift check"
"$ROOT_DIR/scripts/verify_coverage_map.sh"

echo "[6/6] GAP metrics regression check"
